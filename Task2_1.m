...
%% 1. System Parameters (From Assignment Guidelines)
...
fprintf('--- Part B Task 1: RIS-Assisted MU-MISO System ---\n');

%% 2. mmWave Cascaded Channel Model Generation (3D Saleh-Valenzuela)
fprintf('Generating mmWave Cascaded Channel (BS -> RIS -> UE)...\n');

% Set random phase shifts for all RIS elements (Assignment Requirement)
omega = rand(N, 1) * 2 * pi;
...
tau_2 = randi([0, 8], L, 1); % Delays in samples (Max total delay = 15 <= CP)
phi_t_2 = rand(L, 1) * pi; theta_t_2 = rand(L, 1) * pi; % AOD/ZOD at RIS

% Anonymous function for BS Uniform Linear Array (ULA) response
a_BS = @(phi) (1/sqrt(M)) * exp(1i * pi * (0:M-1)' * sin(phi));

% Pre-compute RIS Uniform Planar Array (UPA) responses
a_RIS_H1 = zeros(N, L); 
a_RIS_H2 = zeros(N, L); 
for l = 1:L
    idx = 1;
...
% Build Frequency Domain Channels for all 64 subcarriers
...
for k_sub = 0:N_fft-1
    ...
    H2_k = zeros(1, N);
    for l = 1:L
        % Sum the multipath contributions with delay phase shifts
...
% Apply Walsh-Hadamard Matrix to make pilots orthogonal across the 4 antennas
W = hadamard(M); 
...
% Combine Frame: [4 Pilot Symbols, 1 Data Symbol]
...
%% 4. Channel Propagation & Interference
fprintf('Transmitting signal through Time-Domain Channel...\n');
rx_signal_clean = zeros(1, size(tx_signal, 2));

% Apply specific time-domain filter for each transmit antenna path
for tx_ant = 1:M
...
%% 5. OFDM Receiver (User Equipment)
...
fprintf('\nSUCCESS! Task 1 Complete. mmWave Channel and MISO-OFDM Chain Validated.\n');
