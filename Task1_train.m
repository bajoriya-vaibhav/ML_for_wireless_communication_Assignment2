...
fprintf('=== Task 3: Training 16-bit Chunked DNN Models ===\n');
fprintf('3 scenarios x 8 chunks = 24 networks total\n\n');

%% train all 3 Scenarios
fprintf('Scenario 1/3: 64 Pilots, With CP...\n');
[info_64] = train_chunked_models('model_64_cp.mat', 64, true, ...
                                  N_fft, N_cp, M, k, pdp, pilot_idx_8);

fprintf('\nScenario 2/3: 8 Pilots, With CP...\n');
train_chunked_models('model_8_cp.mat', 8, true, ...
                      N_fft, N_cp, M, k, pdp, pilot_idx_8);

fprintf('\nScenario 3/3: 64 Pilots, No CP...\n');
train_chunked_models('model_64_nocp.mat', 64, false, ...
                      N_fft, N_cp, M, k, pdp, pilot_idx_8);

fprintf('\n=== Task 3 Complete! All models saved. ===\n');

%% pllot Validation Loss (from Scenario 1, Chunk 1)
...
function [info_chunk1] = train_chunked_models(filename, num_pilots, use_cp, ...
    ...
    fprintf('  Generating %dk training samples...\n', num_samples/1000);
    for i = 1:num_samples
        ...
        % extract pilot and data blocks (DFT)
