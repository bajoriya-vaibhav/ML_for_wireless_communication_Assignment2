...
N_fft = 64;             % # of subcarriers
N_cp = 16;              % CP Length
M = 4;                  % QPSK
k = log2(M);            % bits per symbol
max_delay = 16;         % max channel delay spread
...
fprintf('--- Executing Task 1: OFDM Tx/Rx Chain & Estimation ---\n\n');

%% WINNER II Channel Generation
% configure Antenna Arrays (SISO for this baseline)
AA = winner2.AntennaArray('ULA', 1, 0.5); 

% configure Layout (1 BS, 1 Mobile Station)
MSIdx = [2];     % MS uses the 2nd antenna array
BSIdx = {1};     % BS uses the 1st antenna array
cfgLayout = winner2.layoutparset(MSIdx, BSIdx, 1, [AA, AA]); 
cfgLayout.ScenarioVector = 4;        % 4 = Bad Urban 
...
% built-in WINNER II Channel System Object
WINNER_chan = comm.WINNER2Channel(cfgWim, cfgLayout);

% discrete-time impulse response by passing a Dirac impulse
impulse_in = complex([1; zeros(N_fft-1, 1)]);
h_raw_full = WINNER_chan(impulse_in);

if iscell(h_raw_full)
    h_raw_matrix = h_raw_full{1}; % Rip the double array out of the cell
else
    h_raw_matrix = h_raw_full;    % Already a double
end
h_raw = h_raw_matrix(:, 1);       % Ensure it's a column vector

% map to standard max_delay (16 taps) to act as our static filter
h_time = zeros(max_delay, 1);
num_taps = min(length(h_raw), max_delay);
h_time(1:num_taps) = h_raw(1:num_taps);

H_true_freq = fft(h_time, N_fft); % ground truth for plotting

% discrete Power Delay Profile (PDP) for MMSE Covariance Matrix
pdp_discrete = abs(h_time).^2;
if sum(pdp_discrete) > 0
    pdp_discrete = pdp_discrete / sum(pdp_discrete);
end

% Compute theoretical R_HH for MMSE based on the exact built-in WINNER II PDP
...
% Channel & Noise (Applying the built-in impulse response)
...
% =========================================================================
% CASE B: 8 PILOTS (Comb-type)
% =========================================================================
fprintf('--- Testing Case B: 8 Pilots ---\n');

num_pilots = 8;
pilot_spacing = N_fft / num_pilots;
pilot_idx = 1:pilot_spacing:N_fft; % Indices: 1, 9, 17, 25, 33, 41, 49, 57

% Extract cross-correlation matrices for 8-pilot MMSE
...
% Map 8 pilots to subcarriers, zero-pad the rest
...
% Channel (using same h_time for fair comparison)
...
% Highlight the 8 pilot locations
...
fprintf('Task 1 complete. Plots generated successfully.\n');