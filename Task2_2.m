% =========================================================================
% MLWC Assignment 2 - Part B: Task 2 (CORRECTED)
% Conventional LS and MMSE Channel Estimation for RIS-assisted MU-MISO
% =========================================================================
clear; clc; close all;
...
fprintf('--- Part B Task 2: LS and MMSE Estimation ---\n');
fprintf('Evaluating SER vs SNR for 16x16 RIS mmWave Channel...\n\n');

%% 2. Pre-compute Channel Statistics for MMSE
max_delay = 16;
tau = 0:max_delay-1;
pdp = exp(-tau/2); 
pdp = pdp / sum(pdp);
...
%% 3. Pre-allocate SER Arrays
ser_ls = zeros(num_snr, 1);
ser_mmse = zeros(num_snr, 1);

% Walsh-Hadamard Matrix for Orthogonal Pilots across M antennas
W = hadamard(M);
...
for si = 1:num_snr
    ...
    for it = 1:num_iter
        ...
        % 2. Data (FIX: 1 Stream broadcasted across 4 antennas)
        ...
        for tx_ant = 1:M
            % Despread Walsh sequence to isolate the specific antenna
        ...
        % Calculate effective channel for the broadcasted data
