...
fprintf('=== Part B Task 3: Training DNN for RIS MU-MISO ===\n');
fprintf('Using 16-bit chunked architecture (same as Part A Task 3)\n');
fprintf('Generating %dk samples (Optimized Vectorized Channel)...\n', num_samples/1000);

%% pre-allocate and vectorize RIS Grid positions
X_data = zeros(num_samples, 256);
Y_data = zeros(num_samples, 128);

% vectorize array element positions for blazing fast channel generation
...
for i = 1:num_samples
    ...
    % Build Frequency Domain Cascaded Channel directly
...
fprintf('Dataset generation complete. Starting chunked training...\n');

%% train 8 Chunk Networks
net_array = cell(1,8);
info_chunk1 = [];
for chunk = 1:8
    ...
    options = trainingOptions('adam', ...
        ...
        'LearnRateSchedule','piecewise', ...    % Drops learning rate over time
        'LearnRateDropPeriod', 15, ...          % Drops every 15 epochs
        'LearnRateDropFactor', 0.5, ...         % Cuts LR in half
        'L2Regularization', 1e-4, ...           % Stronger weight penalty
        'ValidationData',   {X_data(num_train+1:end,:), Y_chunk(num_train+1:end,:)}, ...
        'ValidationFrequency', 50, ...
        'Plots',    'none', ...
        'Verbose',  true); % Turned to true so you can watch it learn
