...
%% loadiing DNN Models from Task 3
fprintf('Loading Part B DNN models...\n');
try
    load('model_partB_dnn.mat'); % Loads 'net_array' and 'saved_pilots'
    fprintf('DNN Models loaded successfully.\n\n');
catch
    error('model_partB_dnn.mat not found! Run train_part_b_task3.m first.');
end
...
fprintf('--- Part B Task 4: Final Evaluation (LS vs MMSE vs DNN) ---\n');
fprintf('Evaluating %d iterations per SNR...\n', num_iter);

%% pre-compute Matrices & Allocations
tau = 0:max_delay-1;
...
for si = 1:num_snr
    ...
    for it = 1:num_iter
        ...
        % PATH A: LS and MMSE (4 Orthogonal Pilots)
        num_pilot_sym = M;
        ...
        % FIX: Demodulate to bits, not integers!
        ...
        err_ls = err_ls + sum(rx_bits_ls ~= data_bits);
        err_mmse = err_mmse + sum(rx_bits_mmse ~= data_bits);
        
        % PATH B: DNN (1 Broadcasted Pilot)
